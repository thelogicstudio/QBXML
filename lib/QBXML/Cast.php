...
class Cast
{
	...
	*/
	...
	{
		//$value = self::_castCharset($value);
		
		if (strlen($value) > $length)
		{
			if ($with_abbrevs)
			{
				$value = self::_castAbbreviations($value);
			}
	...
	static public function cast($type_or_action, $field, $value, $use_abbrevs = true, $htmlspecialchars = true)
	{
		...
		*/
		...
		{
			$class = __NAMESPACE__.'\\Requests\\' . $types[$type_or_action];
			$schema = new $class();
		}
		else if (isset($types3[$type_or_action]))		// substr -3
		{
			$class = __NAMESPACE__.'\\Requests\\' . $types3[$type_or_action];
			$schema = new $class();
		}
		else if (isset($types5[$type_or_action]))		// substr -5
		{
			$class = __NAMESPACE__.'\\Requests\\' . $types5[$type_or_action];
			$schema = new $class();
		}
		//else
		//{
			...
			if ($schema->exists($field))
			{
				switch ($schema->dataType($field))
				{
					case QUICKBOOKS_DATATYPE_STRING:
						...
						{
							$value = self::_castTruncate($value, $maxlength, $use_abbrevs);
						}
