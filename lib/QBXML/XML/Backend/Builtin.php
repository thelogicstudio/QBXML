<?php
	...
	namespace TheLogicStudio\QBXML\XML\Backend;

	use TheLogicStudio\QBXML\XML;
	use TheLogicStudio\QBXML\XML\Backend;


class Builtin implements \TheLogicStudio\QBXML\XML\Backend
{
	protected $_xml;
	...
	{
		$base = new \TheLogicStudio\QBXML\XML\Node('root');
		...
		return new \TheLogicStudio\QBXML\XML\Document(current($tmp));
	}
	...
	protected function _parseHelper($xml, &$Root, &$errnum, &$errmsg, $indent = 0)
	{
		...
		foreach ($dstack as $node)
		{
			...
			$Node = new \TheLogicStudio\QBXML\XML\Node($tag);
