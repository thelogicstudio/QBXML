...
class Simplexml implements Backend
{
	protected $_xml;
	protected $_root;
	...
	public function parse(&$errnum, &$errmsg)
	{
		...
		{
			$Root = new \TheLogicStudio\QBXML\XML\Node($this->_root->getName());
			
			$Root = $this->_parseHelper($Root, $this->_root);
			
			//exit;
			return new \TheLogicStudio\QBXML\XML\Document($Root);
		}
	...
	protected function _parseHelper($Base, $simplexml_node, $data = '')
	{
		...
		foreach ($children as $simplexml_child)
		{
			...
			$Child = new \TheLogicStudio\QBXML\XML\Node($simplexml_child->getName());
